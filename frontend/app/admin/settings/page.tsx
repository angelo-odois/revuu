...
export default function SettingsPage() {
  ...
  const hasApiAccess = user?.plan === "business";
  ...
  const loadData = async () => {
    try {
      ...
      // Load profile to get branding preference
      try {
        const profile = await api.getMyProfile(token) as { showBranding?: boolean } | null;
        if (profile && typeof profile.showBranding === "boolean") {
          setShowRevuuBranding(profile.showBranding);
        }
      } catch (error) {
  ...
  const saveBranding = async (showBranding: boolean) => {
    const token = await getValidToken();
    if (!token) return;

    setSavingBranding(true);
    try {
      await api.saveProfile({ showBranding }, token);
      ...
      setShowRevuuBranding(!showBranding);
    } finally {
  ...
  const handleCreateApiKey = async () => {
    ...
    try {
      ...
      toast({ title: "Chave API criada", description: "Copie a chave agora - ela não será exibida novamente.", variant: "success" });
    } catch (error) {
  ...
  const handleDeleteApiKey = async (id: string) => {
    if (!confirm("Tem certeza que deseja excluir esta chave? Aplicações usando esta chave deixarão de funcionar.")) return;

    try {
      setApiKeys(apiKeys.filter((k) => k.id !== id));
      toast({ title: "Chave excluída", variant: "success" });
    } catch (error) {
          ...
          <TabsContent value="api" className="space-y-6">
                ...
                <CardContent className="pt-6">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Integre seu portfólio com outras aplicações usando nossa API REST.
                      </p>
                    </div>
                    <Link href="/admin/subscription">
                      <Button className="gap-2">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Nossa API REST permite integrar seu portfólio com outras aplicações.
                    {!hasApiAccess && " Faça upgrade para Business para ter acesso."}
                  </p>
                  ...
                  <div className="grid gap-2 text-sm">
                    ...
                    <ul className="list-disc list-inside text-muted-foreground space-y-1">
                      <li>GET /v1/portfolio/:username - Dados do portfólio</li>
                      <li>GET /v1/analytics - Métricas e estatísticas</li>
                      <li>POST /v1/pages - Criar nova página</li>
                      <li>PUT /v1/pages/:id - Atualizar página</li>
                      <li>DELETE /v1/pages/:id - Excluir página</li>
                    </ul>
                  </div>
              ...
              <>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Chaves API permitem acesso programático ao seu portfólio
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-col sm:flex-row gap-4">
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          placeholder="Ex: Meu App, Integração CRM"
                          className="mt-1"
                        />
                      </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg">Suas Chaves API</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {apiKeys.length === 0 ? (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground mt-1">
                                ...
                                {apiKey.lastUsed && ` | Último uso: ${new Date(apiKey.lastUsed).toLocaleDateString("pt-BR")}`}
                              </p>
                            </div>
                            <Button
                              variant="ghost"
